...
class Ccc_Credit_Model_Credit_Transaction  extends Mage_Core_Model_Abstract {
    ...
    public function setTransaction($observer)
    {
        ...
        $oldCurrentBalance = $customer->getOrigData('credit_balance') + $customer->getOrigData('credit_limit_allowed');

        $newCurrentBalance = $customer->getData('credit_balance') + $customer->getData('credit_limit_allowed');
        ...
        if ($oldCurrentBalance != $newCurrentBalance) {
            ...
            $this->setBalanceBeforeTransaction($oldCurrentBalance);

            $this->setBalanceAfterTransaction($newCurrentBalance);
